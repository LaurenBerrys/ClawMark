...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  test("includes the managed legacy session fallback under state root", () => {
    const prev = process.env.OPENCLAW_STATE_ROOT;
    process.env.OPENCLAW_STATE_ROOT = tmpDir;
    try {
      expect(resolveSessionTranscriptCandidates("legacy-session", undefined)).toContain(
        path.join(tmpDir, "sessions", "legacy-session.jsonl"),
      );
    } finally {
      if (prev === undefined) {
        delete process.env.OPENCLAW_STATE_ROOT;
      } else {
        process.env.OPENCLAW_STATE_ROOT = prev;
      }
    }
  });

  test("prefers the managed agent transcript path when OPENCLAW_INSTANCE_ROOT is set", () => {
    const previousInstanceRoot = process.env.OPENCLAW_INSTANCE_ROOT;
    const previousStateDir = process.env.OPENCLAW_STATE_DIR;
    delete process.env.OPENCLAW_STATE_DIR;
    process.env.OPENCLAW_INSTANCE_ROOT = "/runtime/openclaw";

    try {
      const candidates = resolveSessionTranscriptCandidates("sess-runtime", undefined);
      ...
      if (previousInstanceRoot === undefined) {
        ...
        process.env.OPENCLAW_INSTANCE_ROOT = previousInstanceRoot;
      }
