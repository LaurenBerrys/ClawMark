...
export function resolveSessionTranscriptCandidates(
  ...
  if (agentId) {
    ...
    pushCandidate(() => resolveSessionTranscriptPath(sessionId));
  }

  const manifest = resolveInstanceManifest({ env: process.env, homedir: os.homedir });
  const legacyDir = path.join(manifest.stateRoot, "sessions");
