...
describe("runtime federation assignments", () => {
  it("materializes a pending assignment into an authoritative local task", async () => {
    await withTempRoot("openclaw-runtime-federation-assignments-materialize-", async (_root, env) => {
  ...
  it("supports authoritative block, reset, and applied transitions", async () => {
    await withTempRoot("openclaw-runtime-federation-assignments-transitions-", async (_root, env) => {
      ...
      const materialized = materializeRuntimeFederationAssignmentTask("assignment-review-pricing", {
        env,
        now: now + 150,
      });
  ...
  it("refuses to materialize assignments when the bound surface disables local task creation", async () => {
    await withTempRoot("openclaw-runtime-federation-assignments-surface-block-", async (_root, env) => {
