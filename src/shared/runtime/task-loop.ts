...
export function normalizeOptionalTaskStatus(value: string | null | undefined): TaskStatus | null {
  const normalized = normalizeText(value).toLowerCase();
  if (!normalized) {
    return null;
  }
...
export function getTaskStatusAliases(value: string | null | undefined): string[] {
  ...
  if (normalized === "waiting_user") {
    return ["waiting_user", "waiting_human"];
  }
...
export function shouldTaskRun(task: TaskQueueInput | null | undefined, now = Date.now()): boolean {
  const status = normalizeTaskStatus(task?.status, "queued");
  if (!isRunnableTaskStatus(status)) {
    return false;
  }
