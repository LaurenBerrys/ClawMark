...
describe("state + config path candidates", () => {
  ...
  it("preserves Windows absolute OPENCLAW_STATE_DIR values", () => {
    const env = {
      OPENCLAW_STATE_DIR: "E:\\OpenClaw\\state",
    } as NodeJS.ProcessEnv;

    expect(resolveStateDir(env, () => "/home/test")).toBe("E:\\OpenClaw\\state");
  });

  it("derives the canonical config path from OPENCLAW_CONFIG_ROOT", () => {
    const env = {
      OPENCLAW_STATE_DIR: "/new/state",
      OPENCLAW_CONFIG_ROOT: "/new/config",
    } as NodeJS.ProcessEnv;

    expect(resolveCanonicalConfigPath(env, "/new/state", () => "/home/test")).toBe(
      path.join("/new/config", "openclaw.json"),
    );
  });
