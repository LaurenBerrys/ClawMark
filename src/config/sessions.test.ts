...
describe("sessions", () => {
  ...
  it("derives session transcripts dir from OPENCLAW_INSTANCE_ROOT", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENCLAW_INSTANCE_ROOT: "/custom/instance" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
    expect(dir).toBe(path.join(path.resolve("/custom/instance"), "agents", "main", "sessions"));
  });

  it("derives default session store path from OPENCLAW_INSTANCE_ROOT", () => {
    const prev = process.env.OPENCLAW_INSTANCE_ROOT;
    process.env.OPENCLAW_INSTANCE_ROOT = "/custom/instance";
    try {
      expect(resolveDefaultSessionStorePath("codex")).toBe(
        path.join(path.resolve("/custom/instance"), "agents", "codex", "sessions", "sessions.json"),
      );
    } finally {
      if (prev === undefined) {
        delete process.env.OPENCLAW_INSTANCE_ROOT;
      } else {
        process.env.OPENCLAW_INSTANCE_ROOT = prev;
      }
    }
  });

  it("derives session transcripts dir from OPENCLAW_AGENTS_ROOT", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENCLAW_AGENTS_ROOT: "/custom/agents" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
    expect(dir).toBe(path.join(path.resolve("/custom/agents"), "main", "sessions"));
  });
  ...
  it("uses OPENCLAW_AGENTS_ROOT when resolving session transcript filenames", () => {
    const previousAgentsRoot = process.env.OPENCLAW_AGENTS_ROOT;
    process.env.OPENCLAW_AGENTS_ROOT = "/custom/agents";
    try {
      const sessionFile = resolveSessionTranscriptPath("sess-3", "codex");
      expect(sessionFile).toBe(
        path.join(path.resolve("/custom/agents"), "codex", "sessions", "sess-3.jsonl"),
      );
    } finally {
      if (previousAgentsRoot === undefined) {
        delete process.env.OPENCLAW_AGENTS_ROOT;
      } else {
        process.env.OPENCLAW_AGENTS_ROOT = previousAgentsRoot;
      }
    }
  });
