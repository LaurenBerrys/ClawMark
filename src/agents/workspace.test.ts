...
import {
  ...
  resolveScopedAgentWorkspaceDir,
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  ...
  it("derives the default workspace from OPENCLAW_INSTANCE_ROOT", () => {
    const dir = resolveDefaultAgentWorkspaceDir(
      { OPENCLAW_INSTANCE_ROOT: "/runtime/openclaw" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
    expect(dir).toBe(path.join(path.resolve("/runtime/openclaw"), "workspace"));
  });

  it("keeps an explicit workspace root stable even when OPENCLAW_PROFILE is set", () => {
    const dir = resolveDefaultAgentWorkspaceDir(
      {
        OPENCLAW_PROFILE: "isolated",
        OPENCLAW_WORKSPACE_ROOT: "/runtime/workspaces/main",
      } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
    expect(dir).toBe(path.resolve("/runtime/workspaces/main"));
  });

  it("adds the active profile suffix when no explicit workspace root is set", () => {
    const dir = resolveDefaultAgentWorkspaceDir(
      {
        OPENCLAW_PROFILE: "isolated",
        OPENCLAW_STATE_ROOT: "/runtime/state",
      } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
    expect(dir).toBe(path.join(path.resolve("/runtime/state"), "workspace-isolated"));
  });

  it("derives scoped agent workspaces from the managed workspace root", () => {
    const dir = resolveScopedAgentWorkspaceDir(
      "coder",
      { OPENCLAW_STATE_ROOT: "/runtime/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
    expect(dir).toBe(path.join(path.resolve("/runtime/state"), "workspace-coder"));
  });
});
