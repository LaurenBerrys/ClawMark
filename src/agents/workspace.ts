...
function hasExplicitWorkspaceRoot(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.OPENCLAW_WORKSPACE_ROOT?.trim() || env.OPENCLAW_WORKSPACE_DIR?.trim());
}
...
export function resolveDefaultAgentWorkspaceDir(
  ...
  if (!hasExplicitWorkspaceRoot(env) && profile && profile.toLowerCase() !== "default") {
    return appendWorkspaceSuffix(manifest.workspaceRoot, profile);
  }
  return manifest.workspaceRoot;
}

export function resolveScopedAgentWorkspaceDir(
  agentId: string,
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  return appendWorkspaceSuffix(resolveDefaultAgentWorkspaceDir(env, homedir), agentId);
}
