---
summary: "Run ClawMark Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want ClawMark running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# ClawMark on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent ClawMark Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "ClawMark 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the ClawMark Gateway in Docker
- Persist `~/.openclaw` + `~/.openclaw/workspace` on the host (survives restarts/rebuilds)
- Access the User Console from your laptop via an SSH tunnel
...
```bash
gcloud projects create my-openclaw-project --name="ClawMark Gateway"
...
## 6) Clone the ClawMark repository
...
If User Console shows `unauthorized` or `disconnected (1008): pairing required`, approve the browser device:
...
ClawMark runs in Docker, but Docker is not the source of truth.
...
1. Create a service account:
   ...
   gcloud iam service-accounts create openclaw-deploy \
     --display-name="ClawMark Deployment"
   ```
