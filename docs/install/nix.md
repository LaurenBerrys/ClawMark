---
summary: "Install ClawMark declaratively with Nix"
read_when:
...
The recommended way to run ClawMark with Nix is via **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** — a batteries-included Home Manager module.
...
ClawMark supports a **Nix mode** that makes configuration deterministic and disables auto-install flows.
...
ClawMark reads JSON5 config from `OPENCLAW_CONFIG_PATH` and stores mutable data in `OPENCLAW_STATE_DIR`.
