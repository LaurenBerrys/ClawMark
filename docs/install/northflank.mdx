...
Deploy ClawMark on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy ClawMark](https://northflank.com/stacks/deploy-openclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy ClawMark now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the ClawMark template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the ClawMark service.
8. Open the public ClawMark URL and complete setup at `/setup`.
9. Open the User Console at `/openclaw`.

## What you get

- Hosted ClawMark Gateway + User Console
...
4. Click **Run setup**.
5. Open the User Console at `https://<your-northflank-domain>/openclaw`
