...
read_when:
  - Installing ClawMark on Windows
...
ClawMark on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- if Scheduled Task creation is denied, ClawMark falls back to a per-user Startup-folder login item and starts the gateway immediately
...
### 2) Install the ClawMark gateway user service
