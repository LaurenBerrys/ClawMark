---
summary: "macOS app flow for controlling a remote ClawMark gateway over SSH"
read_when:
  - Setting up or debugging remote mac control
title: "Remote Control"
---

# Remote ClawMark (macOS ⇄ remote host)

This flow lets the macOS app act as a full remote control for a ClawMark gateway running on another host (desktop/server). It’s the app’s **Remote over SSH** (remote run) feature. All features—health checks, Voice Wake forwarding, and Web Chat—reuse the same remote SSH configuration from _Settings → General_.

## Modes

- **Local (this Mac)**: Everything runs on the laptop. No SSH involved.
- **Remote over SSH (default)**: the `openclaw` CLI runs on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
2. Under **OpenClaw runs** (current settings label), pick **Remote over SSH** and set:
