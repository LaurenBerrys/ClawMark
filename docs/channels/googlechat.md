...
6. **Enable the app status**:
   ...
   - Click **Save** again.
7. Configure ClawMark with the service account path + webhook audience:
...
Google Chat webhooks require a public HTTPS endpoint. For security, **only expose the `/googlechat` path** to the internet. Keep the ClawMark dashboard and other sensitive endpoints on your private network.
...
With this config, any request to `your-domain.com/` will be ignored or returned as 404, while `your-domain.com/googlechat` is safely routed to ClawMark.
...
1. Google Chat sends webhook POSTs to the gateway. Each request includes an `Authorization: Bearer <token>` header.
   - ClawMark verifies bearer auth before reading/parsing full webhook bodies when the header is present.
...
2. ClawMark verifies the token against the configured `audienceType` + `audience`:
