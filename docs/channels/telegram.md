...
- DM messages can carry `message_thread_id`; ClawMark routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ClawMark can stream partial replies in real time:
    ...
    - DM: ClawMark keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: ClawMark keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), ClawMark falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, ClawMark skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, ClawMark automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, ClawMark retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - ClawMark pins the spawn confirmation message in-topic after a successful bind.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, ClawMark enqueues system events like:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, ClawMark preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, ClawMark now retries these as recoverable network errors.
...
- Multi-account precedence:
  ...
  - If neither is set, ClawMark falls back to the first normalized account ID and `openclaw doctor` warns.
