...
ClawMark routes replies **back to the channel where a message came from**. The
...
ClawMark infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, ClawMark still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when ClawMark would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
