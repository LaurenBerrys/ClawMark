...
You will need to create a new application with a bot, add the bot to your server, and pair it to ClawMark. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "ClawMark".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your ClawMark agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to ClawMark in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with ClawMark. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Step 0: Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running ClawMark before messaging your agent.
...
```
    ...
    If ClawMark is already running as a background service, use `openclaw gateway restart` instead.

  </Step>

  <Step title="Configure ClawMark and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your ClawMark agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
Discord forum and media channels only accept thread posts. ClawMark supports two ways to create them:
...
ClawMark supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- ClawMark adds a trigger button automatically
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in ClawMark config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces ClawMark auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    ClawMark can stream draft replies by sending a temporary message and editing it as text arrives.
...
```
    ...
    enabled for Discord, ClawMark skips the preview stream to avoid double streaming.
...
ClawMark uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires Carbon component instances), while legacy `embeds` remain available but are not recommended.
...
ClawMark can join Discord voice channels for realtime, continuous conversations. This is separate from voice message attachments.
...
- ClawMark also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. ClawMark generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; ClawMark converts to OGG/Opus when needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep ClawMark current (`openclaw update`) so the Discord voice receive recovery logic is present
