...
- ClawMark installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "ClawMark"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure ClawMark and restart the gateway.
...
2. Link a bot account:
   - `signal-cli link -n "ClawMark"` then scan the QR in Signal.
...
4. Configure ClawMark, restart gateway, verify channel:
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point ClawMark at it:
...
This skips auto-spawn and the startup wait inside ClawMark. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- **Typing indicators**: ClawMark sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, ClawMark forwards read receipts for allowed DMs.
