---
summary: "Run ClawMark with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run ClawMark against a local SGLang server
...
ClawMark can connect to SGLang using the `openai-completions` API.

ClawMark can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, ClawMark will query:
