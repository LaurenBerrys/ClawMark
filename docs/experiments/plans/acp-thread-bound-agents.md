...
This plan defines how ClawMark should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- ClawMark core owns ACP control plane concerns
...
ClawMark should not reimplement ACP transport internals in core.
ClawMark should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in ClawMark, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under the ClawMark state dir (`~/.openclaw`)
...
- ACP event stream is normalized to ClawMark reply chunks
...
For the first production backend (`extensions/acpx`), ClawMark and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses the ClawMark `sessionKey`)
...
- ClawMark consumes ndjson events from `acpx --format json --json-strict`
