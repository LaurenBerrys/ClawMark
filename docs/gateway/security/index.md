...
> ClawMark is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
ClawMark security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
ClawMark is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
ClawMark assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.openclaw`, including `openclaw.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- ClawMark can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want ClawMark, use one VPS/host per user.
...
ClawMark is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
If you run `--deep`, ClawMark also attempts a best-effort live Gateway probe.
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full ClawMark state                                 | filesystem perms on `~/.openclaw`                                                                 | yes      |
...
| `gateway.control_ui.allowed_origins_required`      | critical      | Non-loopback User Console without explicit browser-origin allowlist                  | `gateway.controlUi.allowedOrigins`                                                                | no       |
...
## User Console over HTTP

The User Console needs a **secure context** (HTTPS or localhost) to generate device
identity. `gateway.controlUi.allowInsecureAuth` is a local compatibility toggle:

- On localhost, it allows User Console auth without device identity when the page
...
Complete `dangerous*` / `dangerously*` config keys defined in ClawMark config
schema:
...
- ClawMark gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from ClawMark responses.
- Detailed deployment guidance is in [Trusted Proxy Auth](/gateway/trusted-proxy-auth#tls-termination-and-hsts).
- For non-loopback User Console deployments, `gateway.controlUi.allowedOrigins` is required by default.
...
ClawMark stores session transcripts on disk under `~/.openclaw/agents/<agentId>/sessions/*.jsonl`.
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If ClawMark cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
- If you don’t want remote execution, set security to **deny** and remove node pairing for that Mac.

## Dynamic skills (watcher / remote nodes)

ClawMark can refresh the skills list mid-session:
...
- If you install plugins from npm (`openclaw plugins install <npm-spec>`), treat it like running untrusted code:
  ...
  - ClawMark uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
By default, ClawMark routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
ClawMark has two separate “who can trigger me?” layers:
...
ClawMark includes explicit bypass flags that disable external-content safety wrapping:
...
This HTTP surface includes the User Console and the canvas host:

- User Console (SPA assets) (default base path `/`)
...
If you run ClawMark with Docker on a VPS, remember that published container ports
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), ClawMark
accepts Tailscale Serve identity headers (`tailscale-user-login`) for User
Console/WebSocket authentication. ClawMark verifies the identity by resolving the
...
- ClawMark will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only ClawMark clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

ClawMark’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default ClawMark limits these tools
...
- Timestamp, gateway host OS + ClawMark version
...
Found a vulnerability in ClawMark? Please report responsibly:
