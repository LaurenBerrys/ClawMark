---
summary: "How ClawMark sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
ClawMark can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - noVNC observer access is password-protected by default; ClawMark emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
ClawMark mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- ClawMark blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
