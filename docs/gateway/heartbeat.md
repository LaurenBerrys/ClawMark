...
- During heartbeat runs, ClawMark treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, ClawMark skips the heartbeat run entirely (no model call).
