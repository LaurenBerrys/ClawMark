...
summary: "Architecture of ClawMark's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in ClawMark
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how ClawMark integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

ClawMark uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, ClawMark directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in ClawMark's local TUI mode)                                             |
...
├── openclaw-tools.ts              # ClawMark-specific tools
...
Image injection is prompt-local: ClawMark loads image refs from the current prompt and
...
2. **Custom Replacements**: ClawMark replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **ClawMark Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures ClawMark's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, ClawMark CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
ClawMark wraps this with `guardSessionManager()` for tool result safety.
...
ClawMark maintains an auth profile store with multiple API keys per provider:
...
ClawMark loads custom pi extensions for specialized behavior:
...
ClawMark also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | ClawMark Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom ClawMark tool suite                                                                     |
