...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a ClawMark Gateway.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the ClawMark gateway or agent instead.                                                                                                                                     |
...
it to drive a ClawMark Gateway session.
...
ClawMark bot over ACP, use `acpx` with its built-in `openclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx openclaw` at `openclaw acp`.
3. Target the ClawMark session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default ClawMark ACP session
acpx openclaw exec "Summarize the active ClawMark session state."

# Persistent named session for follow-up turns
acpx openclaw sessions ensure --name codex-bridge
acpx openclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my ClawMark work agent for recent context relevant to this repo."
```
...
For a repo-local ClawMark checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an ClawMark agent without scraping a terminal.
...
In Zed, open the Agent panel and select “ClawMark ACP” to start a thread.
