...
- On Windows, if ACL verification is unavailable for a provider path, ClawMark fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
