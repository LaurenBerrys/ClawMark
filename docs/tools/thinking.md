...
- Provider notes:
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; ClawMark normalizes incompatible values to `auto`.
...
- ClawMark resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode applies the same low-latency profile on Codex Responses. ClawMark keeps one shared `/fast` toggle across both auth paths.
...
- Anthropic fast mode is API-key only. ClawMark skips Anthropic service-tier injection for Claude setup-token / OAuth auth and for non-Anthropic proxy base URLs.
