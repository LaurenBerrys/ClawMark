---
title: Lobster
summary: "Typed workflow runtime for ClawMark with resumable approval gates."
description: Typed workflow runtime for ClawMark — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets ClawMark run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: ClawMark runs one Lobster tool call and gets a structured result.
...
ClawMark launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In ClawMark tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the ClawMark Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), ClawMark keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls ClawMark tools that do.
