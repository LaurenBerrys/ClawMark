...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let ClawMark run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask ClawMark in plain language to "run this in Codex" or "start Claude Code in a thread", ClawMark should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want ClawMark-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | ClawMark native sub-agent runtime  |
...
- ClawMark binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, ClawMark returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning ClawMark agent id.
...
- ClawMark ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, ClawMark uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, ClawMark may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new ClawMark session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the ClawMark sandbox.
...
If no target resolves, ClawMark returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, ClawMark returns a clear unsupported-control error.
...
When ClawMark uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal ClawMark `agentId` path).
...
- Relative paths resolve from ClawMark workspace directory.
...
- ClawMark startup remains non-blocking while the backend health check runs.
...
> **Important:** ClawMark currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
