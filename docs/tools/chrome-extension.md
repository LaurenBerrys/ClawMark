---
summary: "Chrome extension: let ClawMark drive your existing Chrome tab"
read_when:
...
The ClawMark Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openclaw-managed Chrome profile.
...
ClawMark then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ClawMark release (npm package) as static files. There is no separate “build” step.

After upgrading ClawMark:

- Re-run `openclaw browser extension install` to refresh the installed files under your ClawMark state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

ClawMark ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ClawMark to control.
...
- `ON`: attached; ClawMark can drive that tab.
...
The CLI intentionally does **not** print a `node_modules` path. Always run `openclaw browser extension install` first to copy the extension to a stable location under your ClawMark state directory.
