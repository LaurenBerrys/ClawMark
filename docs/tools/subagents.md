...
- For manual spawns, delivery is resilient:
  - ClawMark tries direct `agent` delivery first with a stable idempotency key.
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, ClawMark uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. ClawMark creates or binds a thread to that session target in the active channel.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, ClawMark falls back to that session's requester when available
