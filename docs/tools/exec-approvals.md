...
- For shell scripts and direct interpreter/runtime file invocations, ClawMark also tries to bind
...
- allowlist entries live in host-local `~/.openclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via User Console / `openclaw approvals allowlist ...`).
...
## User Console editing

Use the **User Console → Nodes → Exec approvals** card to edit defaults, per‑agent
...
The User Console and macOS app resolve it via `exec.approval.resolve`, then the gateway forwards the
...
- Direct shell script and direct runtime file forms are best-effort bound to one concrete local
  file snapshot.
- If ClawMark cannot identify exactly one concrete local file for an interpreter/runtime command
...
These clients are opt-in. If a channel does not have exec approvals enabled, ClawMark does not treat
...
topics, ClawMark preserves the topic for the approval prompt and the post-approval follow-up.
