---
title: "Memory"
summary: "How ClawMark memory works (workspace files + automatic memory flush)"
read_when:
...
ClawMark memory is **plain Markdown in the agent workspace**. The files are the
...
ClawMark exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, ClawMark triggers a **silent,
...
ClawMark can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, ClawMark auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. ClawMark
...
BM25 + vectors + reranking. Markdown stays the source of truth; ClawMark shells
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, ClawMark retries with `qmd query`. If QMD fails or the binary is
  missing, ClawMark automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- ClawMark does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - ClawMark sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index ClawMark
    uses), run a one-off query with the agent’s XDG dirs.

    ClawMark’s QMD state lives under your **state dir** (defaults to `~/.openclaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    ClawMark uses:

    ```bash
    # Pick the same state dir ClawMark uses
...
- When `scope` denies a search, ClawMark logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, ClawMark exports sanitized session
...
- If `memorySearch.multimodal.enabled = true`, ClawMark also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

### Multimodal memory files (Gemini image + audio)

ClawMark can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> ClawMark will automatically reindex when it detects a model or dimension change.
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, ClawMark automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, ClawMark combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, ClawMark falls back to vector-only search.
...
ClawMark can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, ClawMark stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, ClawMark logs the
