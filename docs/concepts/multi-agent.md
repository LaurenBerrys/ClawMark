...
If you do nothing, ClawMark runs a single agent:
...
- If you later add the same binding for the same agent with an explicit account id, ClawMark upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, ClawMark falls back
