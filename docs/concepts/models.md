...
ClawMark selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models ClawMark can use (plus aliases).
...
Marker persistence is source-authoritative: ClawMark writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever ClawMark regenerates `models.json`, including command-driven paths like `openclaw agent`.
