...
ClawMark ships with the pi‑ai catalog. These providers require **no**
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in ClawMark because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; ClawMark maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like ClawMark.
...
- Compatibility: legacy ClawMark config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
...
- Caution: Antigravity and Gemini CLI OAuth in ClawMark are unofficial integrations. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, ClawMark defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), ClawMark forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- If `baseUrl` is empty/omitted, ClawMark keeps the default OpenAI behavior (which resolves to `api.openai.com`).
