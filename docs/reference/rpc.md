...
ClawMark integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- ClawMark owns lifecycle when `channels.signal.autoStart=true`.
...
- ClawMark spawns `imsg rpc` as a child process (legacy iMessage integration).
