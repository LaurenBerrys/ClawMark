...
- With Anthropic API-key auth profiles, ClawMark seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, ClawMark injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
ClawMark exposes dedicated cache-trace diagnostics for embedded agent runs.
