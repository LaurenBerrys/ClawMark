---
summary: "How ClawMark builds prompt context and reports token usage + costs"
read_when:
...
ClawMark tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

ClawMark assembles its own system prompt on every run. It includes:
...
For images, ClawMark downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from ClawMark session logs.
...
`cacheWrite`. If pricing is missing, ClawMark shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. ClawMark can
...
Anthropic's 1M context window is currently beta-gated. ClawMark can inject the
...
ClawMark skips the `context-1m-*` beta header because Anthropic currently
