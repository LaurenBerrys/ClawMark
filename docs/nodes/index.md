...
- For direct shell/runtime file executions, ClawMark also best-effort binds one concrete local
...
```bash
openclaw nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from ClawMark"}'
```
...
ClawMark can run a **headless node host** (no UI) that connects to the Gateway
