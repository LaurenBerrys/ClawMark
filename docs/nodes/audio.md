...
- **Media understanding (audio)**: If audio understanding is enabled (or auto‑detected), ClawMark:
...
ClawMark auto-detects in this order and stops at the first working option:
...
If no proxy env vars are set, direct egress is used. If proxy config is malformed, ClawMark logs a warning and falls back to direct fetch.

## Mention Detection in Groups

When `requireMention: true` is set for a group chat, ClawMark now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, ClawMark performs a "preflight" transcription.
...
- For `parakeet-mlx`, if you pass `--output-dir`, ClawMark reads `<output-dir>/<media-basename>.txt` when `--output-format` is `txt` (or omitted); non-`txt` output formats fall back to stdout parsing.
