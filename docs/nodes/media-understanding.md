...
ClawMark can **summarize inbound media** (image/audio/video) before the reply pipeline runs. It auto‑detects when local tools or provider keys are available, and can be disabled or customized. If understanding is off, models still receive the original files/URLs as usual.
...
- If `<capability>.enabled: true` but no models are configured, ClawMark tries the
  **active reply model** when its provider supports the capability.

### Auto-detect media understanding (default)

If `tools.media.<capability>.enabled` is **not** set to `false` and you haven’t
configured models, ClawMark auto-detects in this order and **stops at the first
working option**:
...
When provider-based **audio** and **video** media understanding is enabled, ClawMark
...
If the proxy value is malformed, ClawMark logs a warning and falls back to direct
fetch.

## Capabilities (optional)

If you set `capabilities`, the entry only runs for those media types. For shared
lists, ClawMark can infer defaults:
...
## Provider support matrix (ClawMark integrations)
...
- `parakeet-mlx` note: with `--output-dir`, ClawMark reads `<output-dir>/<media-basename>.txt` when output format is `txt` (or unspecified); non-`txt` formats fall back to stdout.
