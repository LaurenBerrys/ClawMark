...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in ClawMark.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in ClawMark. See [Webhook Hooks](/automation/webhook) or use `openclaw webhooks` for Gmail helper commands.
...
- Extend ClawMark's behavior without modifying core code

## Getting Started

### Bundled Hooks

ClawMark ships with four bundled hooks that are automatically discovered:
...
3. **Bundled hooks**: `<openclaw>/dist/hooks/bundled/` (shipped with ClawMark)
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before ClawMark persists them.
