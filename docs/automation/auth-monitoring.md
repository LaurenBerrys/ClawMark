...
ClawMark exposes OAuth expiry health via `openclaw models status`. Use that for
...
- `scripts/claude-auth-status.sh`: Claude Code + ClawMark auth checker (full/json/simple).
...
- `scripts/termux-sync-widget.sh`: sync Claude Code creds → ClawMark.
