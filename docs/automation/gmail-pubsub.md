---
summary: "Gmail Pub/Sub push wired into ClawMark webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to ClawMark
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> ClawMark

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> ClawMark webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- ClawMark hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the ClawMark helper to wire everything together (installs deps on macOS via brew):
...
Path note: when `tailscale.mode` is enabled, ClawMark automatically sets
...
- `--hook-url` points to ClawMark `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to ClawMark.
