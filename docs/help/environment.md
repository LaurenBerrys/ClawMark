---
summary: "Where ClawMark loads environment variables and the precedence order"
read_when:
...
ClawMark pulls environment variables from multiple sources. The rule is **never override existing values**.
...
ClawMark also injects context markers into spawned child processes:
...
- `COLORFGBG`: if your terminal exports it, ClawMark uses the background color hint to auto-pick the TUI palette.
...
ClawMark supports two env-driven patterns:
...
| Variable               | Purpose                                                                                                                                                                          |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | Override the home directory used for all internal path resolution (`~/.openclaw/`, agent dirs, sessions, credentials). Useful when running ClawMark as a dedicated service user. |
