---
summary: "Frequently asked questions about ClawMark setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up ClawMark?](#whats-the-recommended-way-to-install-and-set-up-clawmark)
  ...
  - [How do I install ClawMark on Linux?](#how-do-i-install-clawmark-on-linux)
  - [How do I install ClawMark on a VPS?](#how-do-i-install-clawmark-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask ClawMark to update itself?](#can-i-ask-clawmark-to-update-itself)
  ...
  - [If I buy a Mac mini to run ClawMark, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-clawmark-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different ClawMark instances?](#can-multiple-people-use-one-whatsapp-number-with-different-clawmark-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run ClawMark on a dedicated machine?](#how-important-is-it-to-run-clawmark-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run ClawMark in a VM and what are the requirements](#can-i-run-clawmark-in-a-vm-and-what-are-the-requirements)
- [What is ClawMark?](#what-is-clawmark)
  - [What is ClawMark, in one paragraph?](#what-is-clawmark-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for ClawMark](#what-are-the-top-five-everyday-use-cases-for-clawmark)
  - [Can ClawMark help with lead gen outreach ads and blogs for a SaaS](#can-clawmark-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can ClawMark run tasks on a schedule or continuously in the background?](#can-clawmark-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with ClawMark saved locally?](#is-all-data-used-with-clawmark-saved-locally)
  - [Where does ClawMark store its data?](#where-does-clawmark-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall ClawMark?](#how-do-i-completely-uninstall-clawmark)
...
- [Config basics](#config-basics)
  ...
  - [Can the ClawMark browser run headless?](#can-the-clawmark-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two ClawMark instances talk to each other (local + VPS)?](#can-two-clawmark-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does ClawMark load environment variables?](#how-does-clawmark-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of ClawMark instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-clawmark-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset ClawMark but keep it installed?](#how-do-i-completely-reset-clawmark-but-keep-it-installed)
  ...
  - [Why doesn't ClawMark reply in a group?](#why-doesnt-clawmark-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do ClawMark, Flawd, and Krill use for models?](#what-do-clawmark-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [How do I run ClawMark in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-clawmark-in-remote-mode-client-connects-to-a-gateway-elsewhere)
  - [The User Console says "unauthorized" (or keeps reconnecting). What now?](#the-user-console-says-unauthorized-or-keeps-reconnecting-what-now)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart ClawMark?](#i-closed-my-terminal-on-windows-how-do-i-restart-clawmark)
...
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose ClawMark to inbound DMs?](#is-it-safe-to-expose-clawmark-to-inbound-dms)
...
This installs ClawMark **from a git checkout**, so the agent can read the code + docs and
...
### What's the recommended way to install and set up ClawMark
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `OPENCLAW_GATEWAY_TOKEN`) into User Console settings.
...
- **Tailscale Serve** (recommended): keep bind loopback, run `openclaw gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy User Console/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `openclaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/` and paste the token in User Console settings.
...
1. Install ClawMark on the new machine.
...
If you still reproduce this on latest ClawMark, track/report it in:
...
### How do I install ClawMark on Linux
...
### How do I install ClawMark on a VPS
...
from your laptop/phone via the User Console (or Tailscale/SSH). Your state + workspace
...
### Can I ask ClawMark to update itself
...
No. You can run ClawMark with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like ClawMark.
...
Yes - via **setup-token**. ClawMark no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so ClawMark can keep replying while a provider is rate-limited.
...
ClawMark supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. ClawMark fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like ClawMark. The onboarding wizard can run the OAuth flow for you.
...
Usually no. ClawMark needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. ClawMark runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run ClawMark can I connect it to my MacBook Pro
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but ClawMark authorization uses numeric IDs only.
...
### Can multiple people use one WhatsApp number with different ClawMark instances
...
If you run ClawMark via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the ClawMark code install. Your state
...
**ClawMark-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run ClawMark on a dedicated machine
...
ClawMark is lightweight. For a basic Gateway + one chat channel:
...
### Can I run ClawMark in a VM and what are the requirements
...
### What is ClawMark in one paragraph

ClawMark is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

ClawMark is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for ClawMark
...
### Can ClawMark help with lead gen outreach ads and blogs for a SaaS
...
ClawMark draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

ClawMark is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use ClawMark when you
...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.openclaw/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.openclaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which ClawMark treats as `<workspace>/skills`.
...
### Can ClawMark run tasks on a schedule or continuously in the background
...
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-clawmark-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. ClawMark can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured ClawMark workspace); ClawMark treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.openclaw/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
ClawMark memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

ClawMark also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, ClawMark auto-selects a provider when it
...
configured and present, ClawMark
...
### Is all data used with ClawMark saved locally

No - **ClawMark's state is local**, but **external services still see what you send them**.
...
### Where does ClawMark store its data
...
### How do I completely uninstall ClawMark
...
`workspace` to the repo root. The ClawMark repo is just source code; keep the
...
ClawMark reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.openclaw/openclaw.json`):
...
- The User Console authenticates via `connect.params.auth.token` (stored in app/UI settings). Avoid putting tokens in URLs.

### Why do I need a token on localhost now

ClawMark enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.
...
- `default`: uses `All your chats, one ClawMark.` every time.
...
### Can the ClawMark browser run headless
...
### Can two ClawMark instances talk to each other local VPS
...
If you want the User Console without SSH, use Tailscale Serve on the VPS:
...
Serve exposes the **Gateway User Console + WS**. Nodes connect over the same Gateway WS endpoint.
...
### How does ClawMark load environment variables

ClawMark reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:
...
does **not** mean your env vars are missing - it just means ClawMark won't load
...
### Is there a way to make a team of ClawMark instances one CEO and many agents
...
### How do I completely reset ClawMark but keep it installed
...
headers like `# Heading`), ClawMark skips the heartbeat run to save API calls.
...
No. ClawMark runs on **your own account**, so if you're in the group, ClawMark can see it.
...
### Why doesn't ClawMark reply in a group
...
ClawMark's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, ClawMark currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
### What do ClawMark, Flawd, and Krill use for models
...
Yes. ClawMark ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
Cooldowns apply to failing profiles (exponential backoff), so ClawMark can keep responding even when a provider is rate-limited or temporarily failing.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), ClawMark will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: ClawMark now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
ClawMark uses provider-prefixed IDs like:
...
ClawMark may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `openclaw models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
`gateway.port` controls the single multiplexed port for WebSocket + HTTP (User Console, hooks, etc.).
...
ClawMark enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `openclaw gateway --port <port>`.

### How do I run ClawMark in remote mode client connects to a Gateway elsewhere
...
### The User Console says unauthorized or keeps reconnecting What now

Your gateway is running with auth enabled (`gateway.auth.*`), but the UI is not sending the matching token/password.

Facts (from code):

- The User Console keeps the token in `sessionStorage` for the current browser tab session and selected gateway URL, so same-tab refreshes keep working without restoring long-lived localStorage token persistence.
...
- In the User Console settings, paste the same token.
...
Profiles also suffix service names (`ai.openclaw.<profile>`; legacy `com.openclaw.*`, `openclaw-gateway-<profile>.service`, `ClawMark Gateway (<profile>)`).
...
- Windows: `schtasks /Query /TN "ClawMark Gateway (<profile>)" /V /FO LIST`
...
### I closed my terminal on Windows how do I restart ClawMark
...
- `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. ClawMark already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [ClawMark assistant setup](/start/openclaw) and [Agent send](/tools/agent-send).
...
### Is it safe to expose ClawMark to inbound DMs
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. ClawMark only replies to chats it receives or to explicit sends you trigger.
...
If it is still noisy, check the session settings in the User Console and set verbose
...
ClawMark blocks **cross-provider** messaging by default. If a tool call is bound
