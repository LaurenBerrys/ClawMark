...
| Desktop Console / One-Install    | `in_progress` | The desktop operator surface is now bootstrap-first: Flutter enters a native-host-backed onboarding/workboard flow, reads runtime connection truth from the host bridge instead of assuming a fixed loopback port, and only opens the full Console once the host reports a ready descriptor-driven session. The macOS desktop host now owns app-private `Application Support` roots (`core/current`, `core/staged`, `core/downloads`, logs, descriptor), dynamic loopback port selection, per-session auth tokens, runtime restarts, log opening, and archive install/promote for `ClawMarkCore`. Flutter now ships a GitHub-Releases-driven `ClawMarkCore` downloader/verifier/installer, keeps the existing operator workboards (task/governance/federation/settings/memory) intact after bootstrap, and routes runtime restart/log actions through the native bridge instead of the gateway. The current Flutter desktop shell has also moved onto a Chinese-first dark design system (`desktop_design.dart`), productized bootstrap/onboarding workboard, redesigned left-rail / center interaction / right workboard layout, cross-page search/filter behavior for task/memory/governance/federation/settings surfaces, and widget-level coverage for localized bootstrap + ready-shell states. Packaging defaults are now bootstrap-only (no embedded `DesktopRuntime` unless explicitly requested), dedicated `ClawMarkCore` packagers/manifests plus a GitHub Actions release workflow are staged in-repo, and the current macOS loop has been re-verified locally with `flutter analyze`, `flutter test`, `flutter build macos --debug`, `flutter build macos`, root `pnpm exec tsc -p tsconfig.json --noEmit`, local `ClawMarkCore` asset packaging/manifest generation, and bootstrap-only macOS packaging. Remaining work is visual high-fidelity polish against the Figma references, real macOS/Windows host verification outside the current sandbox-limited Codex session, signed distribution, and Windows-native host parity. |
